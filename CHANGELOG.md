...
### Added
- MIT `LICENSE` and author credit (Kelly O) in README and `version.py`.

### Removed
- Dead code and unused modules (empty stubs, `converter.py`, the standalone
  `voices.py` Voice Studio, and the abandoned `makebook.py` prototype).
- Legacy/generated content from the repo and its history.
