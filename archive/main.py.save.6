...
def convert(book):
    try:
        ...
        if book.suffix.lower() == ".epub":
            status.set("Converting EPUB to text...")
            subprocess.run(["ebook-convert", str(book), str(txt_file)], check=True)
        elif book.suffix.lower() == ".txt":
            shutil.copy(book, txt_file)
        else:
            messagebox.showerror("BookBuilder", "Use EPUB or TXT for now.")
            return
if book.suffix.lower() == ".epub":
    status.set("Converting EPUB to text...")
    subprocess.run(
        ["ebook-convert", str(book), str(txt_file)],
        check=True
    )

elif book.suffix.lower() == ".pdf":
    status.set("Converting PDF to text...")
    subprocess.run(
        ["pdftotext", "-layout", str(book), str(txt_file)],
        check=True
    )
